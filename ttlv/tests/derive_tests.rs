...
fn test_derive_vec_string_two() {
    ...
    let bytes = [
        ...
        0x42, 0x00, 0x94, 0x07, 0x00, 0x00, 0x00, 0x05, 0x68, 0x65, 0x6C, 0x6C, 0x6F, 0x00, 0x00,
        0x00, // UniqueIdentifier="hello"
        0x42, 0x00, 0x94, 0x07, 0x00, 0x00, 0x00, 0x05, 0x77, 0x6F, 0x72, 0x6C, 0x64, 0x00, 0x00,
        0x00, // UniqueIdentifier="world"
    ];
    let mut reader = TtlvReader::new(&bytes);
    let list = StringList::parse(&mut reader).unwrap();
...
fn test_tagged_enum_primitive_variant() {
    ...
    let bytes = [
        ...
        0x42, 0x00, 0x0A, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x42, 0x00, 0x2A, 0x00, 0x00, 0x00,
        0x00, // AttributeName Enum = Tag::CryptographicLength (0x42002A)
        0x42, 0x00, 0x2A, 0x02, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00,
        0x00, // CryptographicLength Integer = 256
    ];
    let mut reader = TtlvReader::new(&bytes);
...
fn test_tagged_enum_enum_variant() {
    ...
    let bytes = [
        0x42, 0x00, 0x08, 0x01, 0x00, 0x00, 0x00, 0x20, // Attribute Structure len=32
        0x42, 0x00, 0x0A, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x42, 0x00, 0x28, 0x00, 0x00, 0x00,
        0x00, // AttributeName Enum = Tag::CryptographicAlgorithm (0x420028)
        0x42, 0x00, 0x28, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,
        0x00, // CryptographicAlgorithm Enum = 3 (Aes)
    ];
    let mut reader = TtlvReader::new(&bytes);
    let attr = TestAttr::parse(&mut reader).unwrap();
    assert_eq!(
        attr,
        TestAttr::CryptographicAlgorithm(CryptographicAlgorithm::Aes)
    );
}
...
fn test_tagged_enum_struct_variant() {
    ...
    let bytes = [
        ...
        0x42, 0x00, 0x0A, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x42, 0x00, 0x53, 0x00, 0x00, 0x00,
        0x00, // AttributeName Enum = Tag::Name (0x420053)
        0x42, 0x00, 0x53, 0x01, 0x00, 0x00, 0x00, 0x20, // Name Structure len=32
        0x42, 0x00, 0x54, 0x02, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x00, // NameType Integer = 1
        0x42, 0x00, 0x55, 0x07, 0x00, 0x00, 0x00, 0x02, 0x68, 0x69, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, // NameValue TextString = "hi"
    ];
    let mut reader = TtlvReader::new(&bytes);
    let attr = TestAttr::parse(&mut reader).unwrap();
    assert_eq!(
        attr,
        TestAttr::Name(TestName {
            name_type: 1,
            name_value: "hi".to_string()
        })
    );
}
...
fn test_tagged_enum_unknown_discriminator() {
    ...
    let bytes = [
        ...
        0x42, 0x00, 0x0A, 0x05, 0x00, 0x00, 0x00, 0x04, 0x99, 0x99, 0x99, 0x99, 0x00, 0x00, 0x00,
        0x00, // AttributeName Enum = 0x99999999 (no matching variant)
    ];
    let mut reader = TtlvReader::new(&bytes);
...
fn test_tagged_enum_vec_field() {
    ...
    let bytes = [
        ...
        0x42, 0x00, 0x08, 0x01, 0x00, 0x00, 0x00, 0x20, 0x42, 0x00, 0x0A, 0x05, 0x00, 0x00, 0x00,
        0x04, 0x00, 0x42, 0x00, 0x2A, 0x00, 0x00, 0x00, 0x00, 0x42, 0x00, 0x2A, 0x02, 0x00, 0x00,
        0x00, 0x04, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
        // second Attribute: CryptographicAlgorithm=Aes
        0x42, 0x00, 0x08, 0x01, 0x00, 0x00, 0x00, 0x20, 0x42, 0x00, 0x0A, 0x05, 0x00, 0x00, 0x00,
        0x04, 0x00, 0x42, 0x00, 0x28, 0x00, 0x00, 0x00, 0x00, 0x42, 0x00, 0x28, 0x05, 0x00, 0x00,
        0x00, 0x04, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00,
    ];
    let mut reader = TtlvReader::new(&bytes);
...
fn test_enum_unknown_discriminant() {
    ...
    let bytes = [
        0x42, 0x00, 0x28, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00,
        0x00,
    ];
    let mut reader = TtlvReader::new(&bytes);
...
fn test_enum_wrong_tag() {
    ...
    let bytes = [
        0x42, 0x00, 0x0D, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,
        0x00,
    ];
    let mut reader = TtlvReader::new(&bytes);
    let err = CryptographicAlgorithm::parse(&mut reader).unwrap_err();
...
fn test_enum_in_struct() {
    ...
    let bytes = [
        ...
        0x42, 0x00, 0x28, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,
        0x00, // CryptographicAlgorithm Enumeration(3)
    ];
    let mut reader = TtlvReader::new(&bytes);
...
fn test_serialize_matches_known_bytes() {
    ...
    let expected = [
        66u8, 0, 120, 1, 0, 0, 0, 48, 66, 0, 119, 1, 0, 0, 0, 32, 66, 0, 106, 2, 0, 0, 0, 4, 0, 0,
        0, 3, 0, 0, 0, 0, 66, 0, 13, 2, 0, 0, 0, 4, 0, 0, 0, 4, 0, 0, 0, 0, 66, 0, 148, 7, 0, 0, 0,
        0,
    ];
    let msg = SerMessage {
        request_header: SerHeader {
...
fn test_discriminator_enum_auto_derive_alpha() {
    ...
    let bytes = [
        ...
        0x42, 0x00, 0x5C, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x00, // Operation Enumeration = 1 (Alpha)
        0x42, 0x00, 0x79, 0x01, 0x00, 0x00, 0x00, 0x10, // RequestPayload struct, len=16
        0x42, 0x00, 0x0D, 0x02, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x2A, 0x00, 0x00, 0x00,
        0x00, // BatchCount Integer = 42
    ];
    let mut reader = TtlvReader::new(&bytes);
...
fn test_discriminator_enum_auto_derive_beta() {
    ...
    let bytes = [
        0x42, 0x00, 0x0F, 0x01, 0x00, 0x00, 0x00, 0x28, // BatchItem struct, len=40
        0x42, 0x00, 0x5C, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00,
        0x00, // Operation Enumeration = 2 (Beta)
        0x42, 0x00, 0x79, 0x01, 0x00, 0x00, 0x00, 0x10, // RequestPayload struct, len=16
        0x42, 0x00, 0x0D, 0x02, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x07, 0x00, 0x00, 0x00,
        0x00, // BatchCount Integer = 7
    ];
    let mut reader = TtlvReader::new(&bytes);
    let item = TestItem::parse(&mut reader).unwrap();
...
fn test_discriminator_enum_explicit_override() {
    ...
    let bytes = [
        0x42, 0x00, 0x0F, 0x01, 0x00, 0x00, 0x00, 0x28, // BatchItem struct, len=40
        0x42, 0x00, 0x5C, 0x05, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00,
        0x00, // Operation Enumeration = 3 (Gamma)
        0x42, 0x00, 0x79, 0x01, 0x00, 0x00, 0x00, 0x10, // RequestPayload struct, len=16
        0x42, 0x00, 0x0D, 0x02, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x63, 0x00, 0x00, 0x00,
        0x00, // BatchCount Integer = 99
    ];
    let mut reader = TtlvReader::new(&bytes);
    let item = TestItem::parse(&mut reader).unwrap();
...
fn test_enum_serialize_in_struct() {
    ...
    let expected: &[u8] = &[
        0x42, 0x00, 0x0F, 0x01, 0x00, 0x00, 0x00, 0x10, 0x42, 0x00, 0x28, 0x05, 0x00, 0x00, 0x00,
        0x04, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00,
    ];
...
fn test_tagged_enum_serialize_struct_variant() {
    ...
    let attr = TestAttr::Name(TestName {
        name_type: 1,
        name_value: "hi".to_string(),
    });
    let mut writer = NestedWriter::new();
