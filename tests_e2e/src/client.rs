...
mod tests {
    ...
    fn test_create_revoke_client() {
        run_e2e_client_test(4, |mut client| {
            let resp = client.create_symmetric_key(CryptographicAlgorithm::AES, 128).unwrap();
            ...
            let _revoke = client.revoke(&resp.unique_identifier, RevocationReason{revocation_reason_code: RevocationReasonCode::CessationofOperation, revocation_message: None}).unwrap();
